...
public class ClickGUI extends Screen {
    ...
    public void onShader(EventShader e) {
        if (mc.screen == this) {
            RenderUtils.drawRoundedRect(e.stack(), windowX, windowY, this.widthAnimation.value, this.heightAnimation.value, 5.0F, 1073741824);
        }
    }
    ...
    public void render(GuiGraphics g, int mouseX, int mouseY, float pPartialTick) {
        ...
        RenderUtils.drawRoundedRect(stack, windowX, windowY, this.widthAnimation.value, this.heightAnimation.value, 5.0F, Colors.getColor(0, 0, 0, 100));
        ...
        for (Category value : Category.values()) {
            ...
            if (yAnimation.target >= 4.0F) {
                harmony.setAlpha(yAnimation.value / 255.0F);
                Fonts.icons.render(stack, value.getIcon(), windowX + 8.0F + xAnimation.value, windowY + 41.0F + height, Color.WHITE, true, 0.4);
                harmony.render(
                        stack, value.getDisplayName(), windowX + 25.0F + xAnimation.value, windowY + 40.0F + height, Color.WHITE, true, 0.4
                );
        ...
        if (this.titleAnimation.value > 5.0F) {
            harmony.setAlpha(this.titleAnimation.value / 255.0F);
            harmony.render(
                    stack, this.titleDisplayName, windowX + 6.0F + this.titleHoverAnimation.value, windowY + 3.0F, Color.WHITE, true, 0.4
            );
        }

        harmony.setAlpha((255.0F - this.titleAnimation.value) / 255.0F);
        harmony.render(stack, Naven.CLIENT_DISPLAY_NAME, windowX + 50.0F - harmony.getWidth(Naven.CLIENT_DISPLAY_NAME, 0.75) / 2.0F, windowY + 5.0F, Color.WHITE, true, 0.75);
        harmony.setAlpha(1.0F);
        if (this.selectedCategory != null) {
            ...
            this.hoveringBack = RenderUtils.isHovering(
                    ...
                    windowX + 8.0F,
                    windowY + 5.0F,
                    windowX + 5.0F + harmony.getWidth(this.titleDisplayName, 0.4),
                    (float) ((double) (windowY + 5.0F) + harmony.getHeight(true, 0.4F))
            );
            if (this.hoveringBack && !this.clickResizeWindow && !this.clickDragWindow) {
        ...
        if (this.categoryModules != null) {
            ...
            for (Module categoryModule : this.categoryModules) {
                ...
                harmony.setAlpha((float) alpha / 255.0F);
                harmony.render(
                        stack,
                        categoryModule.getName(),
                        windowX + 13.0F,
                        windowY + 25.0F + renderModuleHeight + this.categoryMotionY.value,
                        Color.WHITE,
                        true,
                        0.4
                );
            ...
            if (totalHeight > windowHeight - 25.0F) {
                ...
                float progress = (float) MathUtils.clamp(-this.categoryMotionY.value / -this.minCatY, 0.0, 1.0);
        ...
        if (this.renderValues != null) {
            ...
            for (Value value : this.renderValues) {
                if (value.isVisible() && value.getValueType() == ValueType.BOOLEAN) {
                    ...
                    font.render(
                            ...
                            value.getName(),
                            windowX + 155.0F + x,
                            windowY + valueHeight + motion + 19.0F + (float) heightOffset,
                            Color.WHITE,
                            true,
                            scale
                    );
            ...
            for (Value valuex : this.renderValues) {
                if (valuex.isVisible() && valuex.getValueType() == ValueType.FLOAT) {
                    ...
                    harmony.render(stack, valuex.getName(), windowX + 140.0F, windowY + valueHeight + motion + 25.0F, Color.WHITE, true, 0.4);
                    String currentValue = (float) Math.round(floatValue.getCurrentValue() * 100.0F) / 100.0F + " / " + floatValue.getMaxValue();
                    harmony.render(
                            stack,
                            currentValue,
                            windowX + windowWidth - harmony.getWidth(currentValue, 0.4) - 15.0F,
                            windowY + valueHeight + motion + 25.0F,
                            Color.WHITE,
                            true,
                            0.4
                    );
            ...
            for (Value valuexx : this.renderValues) {
                if (valuexx.isVisible() && valuexx.getValueType() == ValueType.MODE) {
                    ...
                    animationx.update(true);
                    harmony.render(stack, valuexx.getName(), windowX + 140.0F, windowY + valueHeight + motion + 25.0F, Color.WHITE, true, 0.4);
                    x = 0.0F;
                    valueHeight += 15.0F;

                    for (int modeIndex = 0; modeIndex < modeValue.getValues().length; modeIndex++) {
                        String mode = modeValue.getValues()[modeIndex];
                        float currentLengthx = harmony.getWidth(mode, 0.4) + 20.0F;
                        ...
                        harmony.render(stack, mode, windowX + 152.0F + x, windowY + valueHeight + motion + 25.0F, Color.WHITE, true, 0.4);
                        x += currentLengthx;
                    }
            ...
            if (valTotalHeight > windowHeight - 25.0F) {
                ...
                float viewable = windowHeight - 25.0F;
                float progress = (float) MathUtils.clamp(-this.moduleValuesMotionY.value / -this.minValueY, 0.0, 1.0);
        ...
        if (this.bindingAnimation.value > 6.0F) {
            ...
            harmony.setAlpha(this.bindingAnimation.value / 255.0F);
            String line1 = "Press a key to bind " + this.bindingModuleName;
            String line2 = "(Press ESC to remove/cancel key bind)";
            harmony.render(
                    stack,
                    line1,
                    windowX + this.widthAnimation.value / 2.0F - harmony.getWidth(line1, 0.6) / 2.0F,
                    (double) windowY + ((double) this.heightAnimation.value - harmony.getHeight(true, 0.6)) / 2.0 - 10.0,
                    Color.WHITE,
                    true,
                    0.6
            );
            harmony.render(
                    stack,
                    line2,
                    windowX + this.widthAnimation.value / 2.0F - harmony.getWidth(line2, 0.4) / 2.0F,
                    (double) windowY + ((double) this.heightAnimation.value - harmony.getHeight(true, 0.4)) / 2.0 + 15.0,
                    Color.WHITE,
                    true,
                    0.4
            );
        ...
        if (this.selectedCategory != null) {
            ...
            Fonts.icons
                    .render(
                            stack,
                            FontIcons.RESIZE,
                            windowX + this.widthAnimation.value - 10.0F,
                            windowY + this.heightAnimation.value - 10.0F,
                            Color.WHITE,
                            false,
                            0.3
                    );
