...
public class Module extends HasValue {
    ...
    public String getName() {
        // 避免在初始化阶段调用 ModuleManager 导致循环依赖
        try {
            Naven navenInstance = Naven.getInstance();
            if (navenInstance == null || navenInstance.getModuleManager() == null) {
                return this.name;
            }
            
            ClickGUIModule module = navenInstance.getModuleManager().getModule(ClickGUIModule.class);
            if (module == null || module.lang == null) {
                return this.name;
            }
